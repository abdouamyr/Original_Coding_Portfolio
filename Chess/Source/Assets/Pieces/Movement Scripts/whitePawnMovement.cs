...
public class whitePawnMovement : MonoBehaviour
{
	...
	void Update () 
	{
		...
		if (whitePieces.whitePieceHasMoved && activePawn)
		{
			deactivatePawn();
		}
		...
		if (gameObject.transform.tag == "Deactivate")
		{
			...
			gameObject.transform.tag = "White";
			deactivatePawn();
		}
	...
	void OnMouseDown ()
	{
		//resets the pawnPos values
		int pawnPosX = -1;
		int pawnPosY = -1;

		//turns on the piece's halo, if it is currently inactive
		if (activePawn == false && turn.playerTurn == ("White"))
		{
			...
			if (x == 1 && tileMover.pieceOnMoveSquare(x+1, y) == false)
			{
			...
			if (pawnPosX < 7 && tileMover.pieceOnMoveSquare((pawnPosX+1), pawnPosY) != true  && myKing.safeIf(x, y,  x+1, y, pieceColor))
			{
			...
			//if the right capture is valid
			if (takePieceRightSquare != null && myKing.safeIf(x, y,  x+1, y-1, pieceColor))
			{
				//highlight the square
				takePieceRightSquare.transform.renderer.material.color = Color.red;
				(takePieceRightSquare.GetComponent("Halo") as Behaviour).enabled = true;
			}	

			//sends the piece of the whitePieceTracker script
		...
		else if (tileMover.activePiece != null && tileMover.activePiece.transform.tag == "Black" && currentSquare.transform.renderer.material.color == Color.red)
		{
	...
	void deactivatePawn () 
	{
		...
		if (takePieceRightSquare != null)
		{
			...
			takePieceRightSquare.transform.renderer.material.color = colorOfTakeLeftSquare;
